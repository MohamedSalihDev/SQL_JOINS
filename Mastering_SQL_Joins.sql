...
--LEFT JOIN shows all records from left table, joined with matching records from
--right table
SELECT
	 m.emp_no
	 ,m.dept_no
...
ORDER BY 
	d.dept_no;
--This does not change the output. It only arranges it differently by the order
--of the table indicated
...
--You don't have to select emp_no in the SELECT clause to be able to use it as 
...
-- 7.1: What is the average salary for the different genders?
...
--The query above is grouped by employee number and gender. Since employee number
--is unique and unrelated to gender, it will give a grouping of the average salary
--for each employee
...
-- Solution to 8.3
SELECT
	ROUND(AVG(s.salary),2) AS average_dept_salary
	,d.dept_name
FROM
	salaries s
JOIN
